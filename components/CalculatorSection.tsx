...
const CalculatorSection: React.FC = () => {
    ...
    const handleCalculate = useCallback(() => {
        ...
        const stop = parseFormattedNumber(stopLossPrice);
        const rr = parseFloat(riskRewardRatio);
        const fee = parseFloat(feeRate);

        if ([capital, usedCapital, risk, entry, stop].some(v => v <= 0)) {
            setError('총 자산, 1회 진입 자산, 허용 손실 비율, 진입가, 손절가는 0보다 커야 합니다.');
            return;
        }
        if (position === 'long' && stop >= entry) {
            setError('Long(롱) 포지션에서는 손절가가 진입가보다 낮아야 합니다.');
            return;
        }
        if (position === 'short' && stop <= entry) {
            setError('Short(숏) 포지션에서는 손절가가 진입가보다 높아야 합니다.');
            return;
        }
        if (isNaN(rr) || rr <= 0) {
        ...
        let leverage = 1;
        if (lossPerUnit > 0) {
            const calculatedLeverage = (maxLoss * entry) / (usedCapital * lossPerUnit);
            leverage = Math.floor(
              !isFinite(calculatedLeverage) || calculatedLeverage < 1
                ? 1
                : calculatedLeverage
            );
        }

        const positionValue = usedCapital * leverage;
        const quantity = positionValue / entry;
        const totalLoss = quantity * lossPerUnit;
        const lossRate = (totalLoss / capital) * 100;

        const maxQuantity = lossPerUnit > 0 ? maxLoss / lossPerUnit : 0;

        const maintenance = 0.005;
        let liquidationPrice = 0;
        if (leverage > 0) {
            liquidationPrice = position === 'long'
                ? entry * leverage / (leverage + 1 - (maintenance * leverage))
                : entry * leverage / (leverage - 1 + (maintenance * leverage));
        }
        ...
        setResults({
            leverage, quantity, maxQuantity, riskRewardTargetPrice, totalLoss, lossRate, liquidationPrice, profitTotal, profitRate, totalFee
        });
        
        let newWarning = '';
        const warnLiqStop = (position === 'long' && liquidationPrice > stop) || (position === 'short' && liquidationPrice < stop);
        if (warnLiqStop) {
            newWarning += '손절가 도달 전 청산될 수 있으니 손절가 또는 레버리지를 조정하세요.';
        }
        if (leverage <= 1 && lossPerUnit > 0) {
            if(newWarning) newWarning += '\n';
            newWarning += '레버리지를 사용하려면 손절가를 조정하세요.';
        }
        setWarning(newWarning || null);

    }, [totalCapital, entryCapital, riskPercent, entryPrice, stopLossPrice, riskRewardRatio, position, feeRate]);
    ...
    return (
        <>
            <div>
                <h2><Icon name="calculator" /> 진입 레버리지 및 수량 계산</h2>
                <div className="input-grid">
                    ...
                    <InputGroup label="손절가" htmlFor="stopLossPrice">
                        <input id="stopLossPrice" type="text" value={stopLossPrice} onChange={(e) => setStopLossPrice(formatNumberString(e.target.value))} inputMode="decimal" />
                    </InputGroup>
                    ...
                    {results && !error && (
                         <>
                            <div className="results-grid">
                                <ResultItem label="레버리지" value={`${results.leverage}x`} />
